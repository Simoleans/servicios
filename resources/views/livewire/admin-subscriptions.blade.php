<div class="py-12">
    ...
    @endif
    @if($modalConfirmEditar)
        @include('servicios.modal.confirm-editarFechas')
    @endif
    @if($modalExtender)
                        ...
                        @foreach($subscriptions as $s)
                            <div class="flex flex-col md:flex-row w-full bg-white dark:bg-gray-800 border-2 border-white shadow-lg rounded-lg overflow-hidden">
                                ...
                                <div class="md:w-2/3 p-4">
                                    ...
                                    <div class="flex item-center flex-row-reverse mt-3 gap-3">
                                        <button wire:click="confirmEditar({{ $s->id }})" class="mt-3 cursor-pointer text-white hover:text-white bg-yellow-700 dark:text-black flex-1 rounded-md py-2 text-center font-semibold hover:bg-yellow-600">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 inline-block mr-1" viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    <div class="flex item-center flex-row-reverse mt-3 gap-3">
                                        <button wire:click="confirmBaja({{ $s->id }})" class="mt-3 cursor-pointer text-white hover:text-white bg-yellow-400 dark:text-black flex-1 rounded-md py-2 text-center font-semibold hover:bg-yellow-600">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 inline-block mr-1" viewBox="0 0 20 20" fill="currentColor">
