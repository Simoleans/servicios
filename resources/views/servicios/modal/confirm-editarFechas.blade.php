<div class="fixed z-10 inset-0 overflow-y-auto ease-out duration-400" >
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
      <form method="POST" wire:submit.prevent="editarFechasSubscription" enctype="multipart/form-data">
          <input type="hidden" wire:model="subscription_id">
          <p class="font-bold py-5 px-5 bg-blue-500 text-xl">¡Editar Fechas!</p>
          <hr>
          <div class="grid grid-cols-2 gap-4 px-4 py-4">
              <div>
                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="inicio">
                  Inicio
                </label>
                <x-jet-input wire:model.defer="fecha_start" type="date" placeholder="Fecha de inicio" required/>
                <x-jet-input-error for="fecha_start" class="mt-2" />
              </div>
              <div>
                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="final">
                  Final
                </label>
                <x-jet-input wire:model.defer="fecha_end" type="date" placeholder="Fecha final" required/>
                <x-jet-input-error for="fecha_end" class="mt-2" />
              </div>
          </div>
         <hr>
